#!/bin/sh
# ============================================================================
# pe-win98-check.posix.sh — POSIX-sh side-by-side rewrite of pe-win98-check.sh
# ============================================================================
# Phase 2 deliverable for docs/plans/pe-check-posix-rewrite.md. Runs unmodified
# under bash, dash, AND busybox-w32 ash. The existing bash pe-win98-check.sh
# stays in place; both can be sourced or invoked side-by-side.
#
# Public interface (preserved from the bash original):
#
#   pe_check_win98 <exe>
...
#
# Direct CLI usage:
#   pe-win98-check.posix.sh <exe> [<exe2> ...]
#   Exit 0 if all pass; 1 if any fail.
#
# Design notes (for the rewriter / reviewer):
...
#     that awk's BEGIN block loads into associative arrays. Two jq calls total,
#     vs. the old bash version's one-jq-per-imported-DLL — meaningful win on
#     Win98 where each fork/exec is ~50-100ms.
#   * Self-locator: $BASH_SOURCE (bash) / $0 (everything else). Symlink-walk
#     dropped — FAT32 has no symlinks; for the cross-tarball Linux symlink
#     wrapper, the install layout puts the JSONs adjacent to the script so
#     resolution succeeds either way.
