...
What works: a small `bb-shim.exe`, copy-renamed once per applet. At [`repro/bb-shim/bb-shim.c`](repro/bb-shim/bb-shim.c) — ~80 lines, builds to ~49 KB statically linked. Each shim does at runtime:
...
2. **Don't shim a name we ship as a standalone**: `sh.exe` (busybox copy), `cmp.exe`/`diff.exe` (diffutils), `patch.exe`, `make.exe`, `ctags.exe`, `gdb.exe`, `muon.exe`. The install loop in [`build-bb-shims.sh`](repro/scripts/build-bb-shims.sh) refuses to overwrite a binary that isn't already a shim copy, but the right move is to keep the SHIM_APPLETS list disjoint from the standalone-tools list in the first place.
...
| [`repro/scripts/build-bb-shims.sh`](repro/scripts/build-bb-shims.sh) | Cross-compiles `bb-shim.c`, PE-verifies the master copy, then copy-installs it under each name in `SHIM_APPLETS`. Refuses to overwrite a binary that isn't already a shim copy (avoids accidentally shadowing `busybox.exe` / `ctags.exe` / etc.). Runs in `EXTRAS_STEPS` between `build-bcrypt-shim` and `verify-extras-package` |
