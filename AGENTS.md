...
- **Auto-invalidate stale install/build sentinels**: `invalidate_if_stale <step> [<input>...]` removes the step's sentinel if the caller script (auto-tracked via `BASH_SOURCE[1]`) or any explicit input file is newer than the sentinel. Call BEFORE `skip_if_done` in any script whose inputs are stable in-tree files we edit directly (the install scripts, the in-tree shim builds, bb-shim, bcrypt-shim, sockdiag, consdiag). Without this, the sentinel stays satisfied forever after the first run and source edits silently don't propagate — see the install-pe-checker staleness postmortem for the canonical instance. Intentionally NOT applied to `build-*` steps that pull from `/work/src/` (upstream source trees — those follow a different cadence driven by `fetch-sources.sh` and config.json refs, not in-tree edits). If you edit a `build-native-*.sh` that doesn't use the helper, you still need to nuke its sentinel by hand to pick up your change.
...
| [`WIN98-MANUAL-CHECKS.md`](WIN98-MANUAL-CHECKS.md) | Active "open / known-broken / doneish" tracker for things shipped that need real-hardware validation. Wine doesn't catch Win9x quirks (§5.8). Where you record actual hardware test outputs |
